...
"#,
    ...
    #[tokio::test]
    async fn combined_portal_create_policy_binds_role_and_scope_in_one_cel_decision() {
        let endpoint = "lightapi.net/rule/createRule/0.1.0";
        let policy = AccessControlRuntime::new(
            Some(AccessControlConfig::default()),
            serde_yaml::from_str::<RuleFileConfig>(
...
ruleBodies:
  create-rule-scope:
    common: Y
    ruleId: create-rule-scope
    ruleName: Bind create role to trusted scope
    ruleType: req-acc
    conditionLanguage: cel
    conditionSecurityProfile: strict
    expression: "(('globalFlag' in toolArguments && toolArguments.globalFlag == true && auditInfo.subject_claims.ClaimsMap.role == 'admin') || (!('globalFlag' in toolArguments) && auditInfo.subject_claims.ClaimsMap.host == toolArguments.hostId && auditInfo.subject_claims.ClaimsMap.role in ['admin', 'org-admin', 'host-admin']))"
endpointRules:
  lightapi.net/rule/createRule/0.1.0:
    req-acc: [create-rule-scope]
"#,
            )
            .expect("combined create policy"),
        );
        let principal = |role: &str, host: &str| AuthPrincipal {
            role: Some(role.to_string()),
            host: Some(host.to_string()),
            claims: json!({"role": role, "host": host}),
            ..AuthPrincipal::default()
        };

        for role in ["admin", "org-admin", "host-admin"] {
            assert_eq!(
                policy
                    .authorize_http_endpoint(
                        endpoint,
                        &[],
                        Some(&principal(role, "tenant-a")),
                        &json!({"hostId": "tenant-a"}),
                        None,
                    )
                    .await,
                AccessDecision::Allowed,
                "{role} should be allowed only in its trusted tenant scope"
            );
        }
        for role in ["org-admin", "host-admin"] {
            assert!(matches!(
                policy
                    .authorize_http_endpoint(
                        endpoint,
        ...
        assert!(matches!(
            policy
                .authorize_http_endpoint(
                    endpoint,
                    &[],
                    Some(&principal("host-admin", "tenant-a")),
                    &json!({"hostId": "tenant-b"}),
                    None,
                )
                .await,
            AccessDecision::Denied(_)
