...
async fn mcp_official_tools_profile_with_live_reload() {
    ...
    let config = json!({"enabled":true,"path":"/mcp","originAllowlist":[format!("http://127.0.0.1:{port}"),format!("http://localhost:{port}")],"protocols":{"stateless":{"maxSubscriptionDurationMs":2000}},"tools":[tool("test_logging_tool"),tool("test_streaming_elicitation"),tool("test_trigger_tool_change"),tool("test_simple_text"),tool("test_image_content"),tool("test_audio_content"),tool("test_embedded_resource"),tool("test_multiple_content_types"),tool("test_error_handling")]});
