...
Implemented through the release-candidate profile and disabled by default.
Phase 7 promotion readiness is implemented; final-spec reconciliation and the
production canary remain blocked until the final revision is published and
...
- not name a pseudo-header, hop-by-hop or proxy-authentication header, HTTP
  framing header, credential header, or gateway-owned MCP routing/session
  header;
...
The following is the locked configuration contract. All new fields have Serde
defaults so existing configuration continues to load unchanged.
...
The legacy version list above is the active compatibility baseline for this
release.
...
For targets that do not explicitly set
`toolMetadata.runtime.allowPrivateTargetHost: true`, SSRF protection is also
enforced by the HTTP client's connection-time DNS resolver. The resolver
rejects the entire lookup when any returned address is non-public, closing the
validation-to-connect DNS rebinding window. Approved internal targets use a
separate client selected from the validated tool metadata; the public client is
never silently downgraded to that path. The public-target resolver cannot be
combined with an HTTP proxy because the proxy would resolve the origin outside
the gateway's connection-time policy; such a client configuration fails
closed.
