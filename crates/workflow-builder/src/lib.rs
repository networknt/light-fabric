...
mod unit_tests {
    ...
    use workflow_core::models::error::OneOfErrorDefinitionOrReference;
    ...
    fn build_workflow_should_work() {
        ...
        let container_ports: HashMap<u16, u16> =
            vec![(8080, 8081), (8082, 8083)].into_iter().collect();
        let container_volumes: HashMap<String, String> =
            vec![("volume-1".to_string(), "/some/fake/path".to_string())]
                .into_iter()
                .collect();
        ...
        let workflow = WorkflowBuilder::new()
            ...
            .do_(listen_task_name, |task| {
                task.listen().to(|e| {
                    e.one().with("key", Value::String("value".to_string()));
                });
            })
            ...
            .do_(switch_task_name, |task| {
                task.switch().case(switch_case_name, |case| {
            ...
            .do_(try_task_name, |task| {
                task.try_()
                    .do_(|tasks| {
                        tasks.do_("fake-wait-task", |subtask| {
                    ...
                    .catch(|catch| {
                            ...
                            .errors(|errors| {
                                errors.with_attributes(catch_errors_attributes.clone());
                            })
                            .when(catch_when)
                            .retry(|retry| {
                                retry
        ...
        assert!(
            workflow
                ...
                .iter()
                .any(|entry| entry
                    .get(&call_task_name.to_string())
                    .map_or(false, |task| {
                        if let TaskDefinition::Call(CallTaskDefinition::Function(call_task)) = task
                        {
                            call_task.call == call_function_name
                                && call_task.with == Some(call_task_with.clone())
                        } else {
                            false
                        }
                    })),
        ...
        assert!(
            workflow
                ...
                .iter()
                .any(
                    |entry| entry.get(&do_task_name.to_string()).map_or(false, |task| {
        ...
        assert!(
            workflow
                ...
                .iter()
                .any(
                    |entry| entry.get(&for_task_name.to_string()).map_or(false, |task| {
                        if let TaskDefinition::For(for_task) = task {
                            for_task.for_.each == for_each
                                && for_task.for_.in_ == for_each_in
                                && for_task.for_.at == Some(for_each_at.to_string())
                        } else {
                            false
                        }
                    })
        ...
        assert!(
            workflow.do_.entries.iter().any(|entry| entry
                .get(&listen_task_name.to_string())
                .map_or(false, |task| {
        ...
        assert!(
            workflow.do_.entries.iter().any(|entry| entry
                .get(&raise_task_name.to_string())
                .map_or(false, |task| {
                    if let TaskDefinition::Raise(raise_task) = task {
                        if let OneOfErrorDefinitionOrReference::Error(error) =
                            &raise_task.raise.error
                        {
        ...
        assert!(
            workflow.do_.entries.iter().any(|entry| entry
                .get(&run_container_task_name.to_string())
                .map_or(false, |task| {
        ...
        assert!(
            workflow.do_.entries.iter().any(|entry| entry
                .get(&run_workflow_task_name.to_string())
                .map_or(false, |task| {
                    if let TaskDefinition::Run(run_task) = task {
        ...
        assert!(
            workflow
                ...
                .iter()
                .any(
                    |entry| entry.get(&set_task_name.to_string()).map_or(false, |task| {
        ...
        assert!(
            workflow.do_.entries.iter().any(|entry| entry
                .get(&switch_task_name.to_string())
                .map_or(false, |task| {
        ...
        assert!(
            workflow
                ...
                .iter()
                .any(
                    |entry| entry.get(&try_task_name.to_string()).map_or(false, |task| {
