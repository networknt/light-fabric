...
mod tests {
    ...
    fn model_not_found_is_safe_for_deployment_fallback() {
        let error = OpenAiCodec.decode_error(
            404,
            None,
    ...
    #[test]
    fn ordinary_not_found_remains_non_retryable() {
        let error = OpenAiCodec.decode_error(
            404,
            None,
            br#"{"error":{"message":"bad route","code":"not_found"}}"#,
        );
        assert_eq!(error.retry, crate::inference::RetryDisposition::Never);
    }
