#!/usr/bin/env python3
"""Live Claude workspace smoke in a disposable three-repository task. No GitHub writes."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    parser.add_argument('--worker', type=Path, default=Path('target/debug/light-claude-worker'))
    ...
    with tempfile.TemporaryDirectory(prefix='workspace-chat-smoke-') as directory:
        ...
        workspace = dict(schemaVersion=1, id='smoke', hostId='host', agents=['claude'], repositories=repositories, operations=['edit', 'review'], indexers={})
        ...
        binding = dict(schemaVersion=1, workspaceId='smoke', hostId='host', environment='dev', runnerId='runner', membershipRevision=revision,
                       authorizationRevision=1, subjects=['steve'], agents=['claude'], intents=['inspect', 'implement'])
        ...
        start = dict(type='start', session_id=str(uuid.uuid4()), turn_id=str(uuid.uuid4()), action_attempt_id=str(uuid.uuid4()), policy_digest='sha256:'+'a'*64,
                     deadline_ms=240000, input=dict(workspaceSpec=dict(request=request, binding=binding, subject='steve', agentId='claude'), adapterContract=contract, adapterQualification=profile['qualification'],claudePolicy=profile['claudePolicy']))
        env = dict(os.environ, LIGHT_WORKSPACE_CONFIG=str(config), LIGHT_CLAUDE_EXECUTABLE=str(args.claude.resolve()), LIGHT_CLAUDE_HOME=str(args.claude_home.resolve()))
