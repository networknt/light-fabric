...
pub trait IO:
    ...
    + Sync
{
    /// Only the pinned TCP/rustls stack has the final socket-write hook.
    fn supports_socket_write_guard(&self) -> bool {
        if let Some(stream) = self.as_any().downcast_ref::<l4::stream::Stream>() {
