...
pub use crate::protocols::request_write_guard::RequestWriteGuard;
...
impl HttpSession {
    ...
    /// Final-write hook for one-shot workflow dispatch. Connection/TLS preparation
    /// has already completed. No request retry may reuse this guard.
    pub async fn write_request_header_guarded(
        &mut self,
        req: Box<RequestHeader>,
        guard: Option<std::sync::Arc<dyn RequestWriteGuard>>,
    ) -> Result<usize> {
        if guard.is_none() {
            return self.write_request_header(req).await;
        }
        // TODO: make sure this can only be called once
        // init body writer
        self.init_req_body_writer(&req);
        ...
        let mut fence = scope.as_ref().map(|s| CancellationFence {
            scope: s.clone(),
            complete: false,
        });
        let write_timeout = self.write_timeout;
        let write_fut = async {
            self.underlying_stream.write_all(to_wire.as_ref()).await?;
            self.underlying_stream.flush().await
        };
        let operation = async {
            match write_timeout {
                Some(t) => timeout(t, write_fut)
                    .await
                    .unwrap_or_else(|_| Err(std::io::Error::from(ErrorKind::TimedOut))),
                None => write_fut.await,
            }
        };
        let result = if let Some(scope) = scope {
            CURRENT.scope(scope, operation).await
        } else {
            operation.await
        };
        result.map_err(|e| Error::because(WriteError, "guarded request header write failed", e))?;
        if let Some(fence) = &mut fence {
            if !fence.scope.wrote() {
                return Error::e_explain(WriteError, "socket guard observed no request write");
            }
            fence.complete = true;
        }
