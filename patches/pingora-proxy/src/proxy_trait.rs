...
pub trait ProxyHttp {
    ...
    /// Return a body that was fully consumed before upstream selection.
    ///
    /// The proxy core calls this hook only after the downstream body reports
    /// completion and before it starts its normal downstream body loop. This
    /// lets a security filter validate a bounded body before opening an
    /// upstream connection without relying on Pingora's fixed retry buffer.
    /// Returning `Some` causes the bytes to pass through
    /// [`Self::request_body_filter`] as one end-of-stream chunk. The callback
    /// may be called again for an upstream retry, so implementations must keep
    /// the verified bytes available for the request lifetime.
    fn prebuffered_request_body(&self, _session: &Session, _ctx: &Self::CTX) -> Option<Bytes>
    where
        Self::CTX: Send + Sync,
    {
        None
