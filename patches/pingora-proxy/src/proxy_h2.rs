    ...
    C: custom::Connector,
{
        ...
        SV::CTX: Send + Sync,
    {
        ...
        // Retry or application pre-buffer: send the complete body before the
        // normal downstream loop. The application hook is consulted only when
        // the original downstream body has already been fully consumed.
        let buffer = session.as_mut().get_retry_buffer().or_else(|| {
            session
                .as_mut()
                .is_body_done()
                .then(|| self.inner.prebuffered_request_body(session, ctx))
                .flatten()
        });
